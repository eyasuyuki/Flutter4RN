...
- Android開発を2009年から行っていまして2016年に某大学の学生向け公式iOS/Androidアプリを開発しました。
...
- これは Dart の公式サイトです。A Tour of the Dart Languageで言語仕様を一通り学習することができます。
- Effective Dartではより良いコードについて学べます。

# 8

- Dartでは全てがオブジェクトです。
- 全てのオブジェクトはクラスのインスタンスです。
- 数値も関数もnullもオブジェクトです。
- 全てObjectクラスのサブクラスです。
...
- これはDartのクラスの例です。Javaのクラスによく似ていますがpublicキーワードはありません。アンダースコアで始まるメンバー変数はprivateアクセスになります。

# 10

- Dartは強い型付けのプログラミング言語です。型推論も行えます。
- この例でxはintと推論されるので文字列を代入するとエラーになります。

# 11

- dynamicという特別な型があります。
- dynamic型の変数にはどんなオブジェクトも代入できます。
...
- リストに対して map, filter, fold そのほかの高階関数が使えます。
...
- これは Twitter で見て思わず保存した画像です。

# 16

- Dart だと全て false になります。
- Dartの等価演算子は適切に動作します。
